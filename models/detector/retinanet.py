...
class RetinaNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # p3_cls = self.classification(features[0])
        # p4_cls = self.classification(features[1])
        # p5_cls = self.classification(features[2])

        # p3_loc = self.regression(features[0])
        # p4_loc = self.regression(features[1])
        # p5_loc = self.regression(features[2])
        # return p3_cls, p4_cls, p5_cls, p3_loc, p4_loc, p5_loc


if __name__ == '__main__':
    from models.backbone.regnet import RegNet
    model = RetinaNet(
        Backbone=RegNet,
